...
bool Context::Init() {
    ...
    m_vertexBuffer = Buffer::CreateWithData(GL_ARRAY_BUFFER, GL_STATIC_DRAW
        ,vertices, sizeof(float)*120); // sizeof(float)*32 16 이런식도 가능

            // GL_DYNAMIC_DRAW 자주 바꿀 때
            // GL_STREAM_DRAW 한번 세팅하고 그리고 바로 버린다.
            // GL_STATIC_DRAW  한번 세팅하고 유지

    m_vertexLayout->SetAttrib(0, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 5, 0);
    m_vertexLayout->SetAttrib(1, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 5, sizeof(float) * 3);
